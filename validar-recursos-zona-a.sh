...
security_group_id=$(aws ec2 describe-security-groups --group-names "secgrp-dev" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)
if [ $? -eq 0 ]; then
  echo "[OK] Security Group secgrp-dev foi criado"
  
  # Validar inbound rule para o security group 'secgrp-dev'
  ...
  # Validar outbound rule para o security group 'secgrp-dev'
  ...
  echo ">[ERRO] Não achei o security group secgrp-dev. Ele foi criado?"
fi
